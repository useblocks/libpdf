...
PDF_WITH_EMPTY_OUTLINE = os.path.join(
    os.path.dirname(__file__), "pdf", "issue-67-example.pdf"
)
PDF_OUTLINE_NO_DEST = os.path.join(os.path.dirname(__file__), "pdf", "pdffill-demo.pdf")
PDF_FIGURE_WITH_INVALID_BBOX = os.path.join(
    os.path.dirname(__file__), "pdf", "pr-138-example.pdf"
)
PDF_CHAPTER_DETECTION = os.path.join(
    os.path.dirname(__file__), "pdf", "DS93-chapter-issue-fix.pdf"
)

# full features PDF
PDF_FULL_FEATURES = os.path.join(os.path.dirname(__file__), "pdf", "full_features.pdf")
PDF_FIGURES_EXTRACTION = os.path.join(
    os.path.dirname(__file__), "pdf", "test_figures_extraction.pdf"
)
PDF_SMART_HEADER_FOOTER_DETECTION = os.path.join(
    os.path.dirname(__file__), "pdf", "test_header_footer_detection.pdf"
)
...
def obj_equal(class_type, instance1, instance2):
    ...
    self_attr = [
        ...
        if (not attr.startswith("__") and not attr.startswith("b_"))
    ]
    other_attr = [
        attr

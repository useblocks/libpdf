...
def test_figures_extract_with_invalid_bbox():
    ...
    assert objects.pdfplumber.pages[0].figures[0]['y0'] == objects.pdfplumber.pages[0].figures[0]['y1']

    assert len(objects.pdfplumber.pages[1].figures) == 1
    assert objects.pdfplumber.pages[1].figures[0]['height'] == 0
    assert objects.pdfplumber.pages[1].figures[0]['y0'] == objects.pdfplumber.pages[1].figures[0]['y1']
...
def test_figures_extraction():
    ...
    assert (
        ...
        < objects.pdfplumber.figures[5]['width'] * objects.pdfplumber.figures[5]['height']
    )
    # check that figure exists no more
    for figure in objects.flattened.figures:
